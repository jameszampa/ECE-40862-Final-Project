...
# Get the most recent 100 posts (Would theoretically break if we did 100 random posts and then added a new user post, can expand number if needed
...
# add a tag thta prevents future processing (i.e. the list will always have only 1 post in it)
...
for media in media_library:
	if str(media.id)in photo_links:	
		...
		local_name = "Temp_Pictures/get_image_test{}.jpeg".format(media_library.index(media))
...
# Temp folder can be used by main.py to import the new images and create a new recognier
